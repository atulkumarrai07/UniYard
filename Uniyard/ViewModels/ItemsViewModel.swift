...
class ItemsViewModel: ObservableObject {
	...
	init() {
		...
		}.assign(to: \.isValidTitle, on: self).store(in: &cancellableSet)
		
		$description.map{description in
			return !self.description.isEmpty
		}.assign(to: \.isValidDescription, on: self).store(in: &cancellableSet)

		
		Publishers.CombineLatest($isValidTitle, $isValidDescription)
			.map { isValidTitle, isValidDescription in
	...
	var descriptionPrompt: String {
		(!description.isEmpty) ? "" : "Description cannot be empty"
	}
